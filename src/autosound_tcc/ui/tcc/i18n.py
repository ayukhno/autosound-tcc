...
answers the first five questions anybody would ask.",
        ...
        "gitTipNoRepo": "The project folder is not a git repository: the tune's history is not kept and there is no backup. The method is to do this itself for a new project (hub #199); until then, run git init in the project folder.",
...
питань, які тобі поставлять.",
        ...
        "gitTipNoRepo": "Тека проєкту не є git-репозиторієм: історія налаштування не зберігається і резервної копії нема. Метод має робити це сам для нового проєкту (hub #199); доти — git init у теці проєкту.",
...
Щоб це читати, обери вище заміри свіпом (sw).",
    ...
    "pl": {
        ...
        "gitTipNoRepo": "Folder projektu nie jest repozytorium git: historia strojenia nie jest zapisywana i nie ma kopii zapasowej. Metoda ma to robić sama dla nowego projektu (hub #199); do tego czasu uruchom git init w folderze projektu.",
    ...
    "de": {
        ...
        "gitTipNoRepo": "Der Projektordner ist kein git-Repository: Die Historie der Abstimmung wird nicht gespeichert, und es gibt keine Sicherung. Die Methode soll das bei einem neuen Projekt selbst tun (hub #199); bis dahin git init im Projektordner ausführen.",
