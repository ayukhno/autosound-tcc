...
answers the first five questions anybody would ask.",
        ...
        "npOnboardingHint": "Use the autosound-tuning skill. This project's intake was filled on the form, so read the project files first and continue from what the method still reports missing (contract.py check). Connect to this project's 'tcc' MCP server (see .mcp.json), and call the reviewer through its call_critic tool rather than running autosound_ai.py yourself. Please work in {language}.",
        "langNameEn": "English",
        ...
        "layoutControl": "Control mode",
        "layoutActive": "Active TCC",
        "layoutControlTip": "The session runs in a terminal: TCC moves to the right half of the screen and shows what it sees of the session, the tables and your panels. Both borders drag, and are remembered.",
        "layoutActiveTip": "Back to the in-app session's layout: the dialog in the middle, the panels at the sides.",
        "layoutBusyTip": "A session is running in this window — stop it first; the control layout hides its dialog.",
        "layoutNoInApp": "Control mode is for a session in a terminal — switch to «Active TCC» to run one here.",
        "ctlMonitor": "Monitoring",
        "ctlTableV": "Table-V",
        "ctlTableO": "Table-O",
        "ctlMonitorNote": "You type in the terminal. These buttons go to the session as signals.",
        "ctlMonitorEmpty": "No process journal yet — the feed fills in as soon as the session in the terminal starts writing.",
        "ctlNoVirtual": "This DSP has no virtual channels.",
        "ctlNoOutputs": "No outputs yet — the register has no configuration.",
        "measNextRound": "next round",
        "measRoundSeriesTip": "Series _{v}: the number these captures carry in their REW titles.",
        "cmpWith": "compare with",
        "cmpWas": "was: {value}",
        "cmpNew": "not in the compared version",
        "dotNone": "nothing is set here",
        "dotSet": "settings are here",
        "dotChg": "changed against {version}",
        "ctlTableI": "Table-I",
        "cmpOtherPreset": "{preset} — another preset",
        "cmpOtherTag": "another configuration",
        "cmpEqChanged": "the bands changed",
        "cmpRowBtn": "⇅ Compare",
        "cmpRowTip": "Show {version}'s EQ under this one, what changed in colour",
        "cmpRowOff": "Pick a version in «compare with» first",
        "bandNew": "new",
        "bandChg": "changed",
        "bandGone": "removed",
        "bandNewTip": "not in {version}",
        "bandGoneTip": "not in the current version",
        "copyEqPairTip": "Two channels are on screen: each one's copy is beside its name.",
        "cfgButton": "Saved in the DSP…",
        "cfgButtonTip": "Record that this configuration went into a preset of the device, under the name you gave it there (SQ-2, FULL-v1). That name is what the comparison and the header show.",
        "cfgTitle": "Saved in the DSP",
        "cfgBlurb": "{version} went into a preset of the device. Under which name, into which slot and preset number, and what for? The method finds which earlier configuration it continues.",
        "cfgName": "Name in the DSP",
        "cfgSlot": "Slot",
        "cfgDspPreset": "Preset number in the device",
        "cfgPurpose": "What it is for",
        "cfgPurposeHint": "sound quality, the full car, the driver's seat…",
        "cfgSave": "Record",
        "cfgNoName": "It needs the name it was saved under in the DSP.",
        "cfgTooOld": "The method on this computer cannot record configurations yet. Update the method.",
        "cfgNoAnswer": "The method did not answer, so nothing was recorded.",
        "cfgSaved": "Recorded: {said}",
        "dspConfigTip": "Saved in the DSP as {code} (slot {slot}{preset}){purpose}.",
        "dspConfigPreset": ", device preset {n}",
        "cmpPrevTip": "Compared by default with the configuration this one continues, not the number below it.",
        "gitSubBroken": "git does not run",
        "gitSubNoRepo": "no history kept",
        "gitSubNoRemote": "no backup",
        "gitSubBehind": "{n} not backed up",
        "gitSubOk": "backed up",
        "gitTipBroken": "git does not run on this computer, so the tune's history is not kept and nothing is backed up. On a Mac: install the Command Line Tools (xcode-select --install).",
        "gitTipNoRepo": "The project folder is not a git repository: the tune's history is not kept and there is no backup. The method makes a new project a repository itself; for this one, «Make it a repository» below runs its command.",
        "gitTipNoRemote": "The history is kept here, but there is no backup: the repository has no remote. A free private one on GitHub: gh repo create {name} --private --source . --push",
        "gitTipBehind": "{n} commits are not on {remote} yet: git push.",
        "gitTipOk": "The history is kept and backed up to {remote}.",
        "gitRow": "Git",
        "gitChanges": "Unsaved changes",
        "gitClean": "none",
        "gitBackup": "Backup",
        "gitNone": "none",
        "gitUnpushed": "Not backed up yet",
        "gitInitBtn": "Make it a repository",
        "gitInitTip": "Runs the method's own command: git init, the .gitignore, one first commit. Nothing leaves this computer.",
        "gitBackupBtn": "Back up to GitHub",
        "gitBackupTip": "Shows the method's command for a free private repository on GitHub, and runs it only when you say yes.",
        "gitBackupAsk": "Create a private repository on GitHub and send the project's history there?\n\n{command}\n\nThe measurements (.mdat) and keys stay on this computer.",
        "gitDone": "Done: {said}",
        "gitFailed": "Did not work: {said}",
        "gitTooOld": "The method on this computer cannot do this yet. Update the method.",
        "gitNoGh": "GitHub's gh is not installed, so the backup cannot be offered. The history is still kept here.",
        "gitGhSignedOut": "gh is not signed in: run gh auth login --web in a terminal, then try again.",
        "extTerminal": "⌁ in the terminal: {who} · {model}",
        "extNoModel": "model not named",
        "extTip": "A session outside this window is connected to TCC ({client} {version}); the model is as the session named itself. The picker beside it is what TCC would start a new session with.",
        "hoOffer": "Phase {phase} has no step left open. The next phase is best started in a clean session.",
        "hoAction": "Check and start clean",
        "hoTitle": "The next phase, in a clean session",
        "hoMissing": "Not yet: the next session would miss something that is only in this conversation.\n\n{missing}\n\nEach line names what fixes it.",
        "hoReady": "Everything the next session needs is on disk.\n\nStart it with the first message «{message}» — it goes on the clipboard.\nKeep open: {resume}",
        "hoStartInApp": "Start a clean session",
        "hoStartTerminal": "Open a terminal",
        "hoCopied": "«{message}» is on the clipboard: send it as the new session's first message, and close the old one.",
        "hoTooOld": "The method on this computer cannot check the handoff yet. Update the method.",
        "tfOffer": "A title in REW is not how the round spells it: {first} (in all {n}). It is fixed by a rename, not a retake.",
        "tfAction": "Fix in REW…",
        "dspNowTip": "{dsp} — the configuration in the processor now: {version} of preset {preset}. Entered in the DSP: {applied}; proposed, not yet entered: {proposed}; measured since: {measured}. The dot is yellow while anything is only proposed, green once all of it is entered (attest).",
        ...
        "menuReviewerKey": "Reviewer key…",
        "menuReviewerKeyTip": "Where the reviewer's API key is kept, and a safe place to enter one. A reviewer signed in through its CLI needs no key.",
        "rkTitle": "Reviewer key",
        "rkBlurb": "The key for the reviewer's API route. The method keeps it in {store}: never in a shell profile, never in the project folder. A reviewer signed in through its own CLI (agy, claude, codex) needs no key at all. The key is never shown here.",
        "rkStore_keychain": "this Mac's Keychain",
        "rkStore_dpapi": "a file encrypted with your Windows login",
        "rkStore_none": "the file critic-env, readable only by you (this computer has no secure store the method can use)",
        "rkUsed_keystore": "in the secure store",
        "rkUsed_file": "in the file critic-env",
        "rkUsed_env": "from the environment: every program sees it",
        "rkUsed_none": "no key",
        "rkPlaceholder": "Paste the key. It stays hidden",
        "rkSave": "Save",
        "rkClose": "Close",
        "rkSaved": "Stored: {where}",
        "rkRefused": "Not stored: {why}",
        "rkNoAnswer": "The method did not answer, so nothing was stored.",
        "rkShell": "{var} is still exported in {file}, line {line}. Every program started from a terminal sees it. The method moves it into the secure store and asks you first:",
        "rkMove": "Move it (opens a terminal)",
        "rkMoveOpened": "Opened in a terminal. Answer the question there, then reopen this window.",
        "rkOld": "The method on this computer cannot store keys yet, so it reads them from {path} or the environment. Update the method; until then nothing changes.",
        ...
        "capImportProposed": "The New name was matched automatically (the round's spelling, or a likely typo). Tick the row if that is the name you mean.",
...
Start TCC again; if it keeps happening, the reason is here and in the log:",
        ...
        "menuGuideTip": "Opens the guide for this version on GitHub: the window, panel by panel, with screenshots.",
        ...
        "gateAskNever": "Never ask",
        "gateAskForeign": "Outside project",
...
Choose sweeps (sw) above to read this.",
        ...
        "editDoneNoBus": "✓ Editing closed — nothing was sent: no session is listening.",
    ...
    "uk": {
        ...
        "leftBankFirstAsk": "У цьому проєкті ще нема реєстру — його описали або скопіювали з іншого авто, а не зняли тут. Прочитай налаштування DSP, як вони зараз стоять у процесорі, і запиши їх ПЕРШОЮ конфігурацією (версія реєстру v_001) пресету {preset}, через гейт, тоді покажи лист налаштувань.",
        ...
        "refreshProjectTip": "Перечитати проєкт з диска (профіль, реєстр)",
...
питань, які тобі поставлять.",
        ...
        "npOnboardingHint": "Скористайся скілом autosound-tuning. Інтейк цього проєкту заповнено у формі, тож спершу прочитай файли проєкту й продовжуй із того, чого метод ще не має (contract.py check). Підключись до MCP-сервера 'tcc' цього проєкту (див. .mcp.json) і викликай рецензента через його інструмент call_critic, а не запуском autosound_ai.py. Працюй {language}.",
        ...
        "layoutControl": "Режим контролю",
        "layoutActive": "Активний TCC",
        "layoutControlTip": "Сесія йде в терміналі: TCC стає на праву половину екрана й показує, що бачить від сесії, таблиці та ваші панелі. Обидві межі тягнуться й запамʼятовуються.",
        "layoutActiveTip": "Назад до розкладки сесії в TCC: діалог посередині, панелі з боків.",
        "layoutBusyTip": "У цьому вікні йде сесія — спершу зупиніть її: режим контролю ховає її діалог.",
        "layoutNoInApp": "Режим контролю — для сесії в терміналі. Щоб запустити її тут, перемкніться на «Активний TCC».",
        "ctlMonitor": "Моніторинг",
        "ctlTableV": "Таблиця-V",
        "ctlTableO": "Таблиця-О",
        "ctlMonitorNote": "Писати — у терміналі. Ці кнопки йдуть сесії сигналом.",
        "ctlMonitorEmpty": "Журналу процесу ще нема — стрічка наповниться, щойно сесія в терміналі почне писати.",
        "ctlNoVirtual": "У цього DSP немає віртуальних каналів.",
        "ctlNoOutputs": "Виходів ще нема — у реєстрі немає конфігурації.",
        "measNextRound": "новий раунд",
        "measRoundSeriesTip": "Серія _{v}: номер, який ці заміри мають у назвах у REW.",
        "cmpWith": "порівняти з",
        "cmpWas": "було: {value}",
        "cmpNew": "у порівнюваній версії цього нема",
        "dotNone": "налаштувань нема",
        "dotSet": "налаштування є",
        "dotChg": "є зміни відносно {version}",
        "ctlTableI": "Таблиця-I",
        "cmpOtherPreset": "{preset} — інший пресет",
        "cmpOtherTag": "інша конфігурація",
        "cmpEqChanged": "смуги змінились",
        "cmpRowBtn": "⇅ Порівняти",
        "cmpRowTip": "Показати EQ {version} під поточним, змінене кольором",
        "cmpRowOff": "Спершу вибери версію в «порівняти з»",
        "bandNew": "нова",
        "bandChg": "змінена",
        "bandGone": "видалена",
        "bandNewTip": "у {version} цієї смуги нема",
        "bandGoneTip": "у поточній версії цієї смуги нема",
        "copyEqPairTip": "На екрані два канали: копіювання кожного — біля його назви.",
        "cfgButton": "Збережено в DSP…",
        "cfgButtonTip": "Записати, що ця конфігурація пішла в пресет пристрою під назвою, яку ти дав їй там (SQ-2, FULL-v1). Цю назву показують порівняння і шапка.",
        "cfgTitle": "Збережено в DSP",
        "cfgBlurb": "{version} пішла в пресет пристрою. Під якою назвою, у який слот і номер пресета, і для чого? Яку попередню конфігурацію вона продовжує, метод знайде сам.",
        "cfgName": "Назва в DSP",
        "cfgSlot": "Слот",
        "cfgDspPreset": "Номер пресета в пристрої",
        "cfgPurpose": "Для чого",
        "cfgPurposeHint": "якість звуку, уся машина, місце водія…",
        "cfgSave": "Записати",
        "cfgNoName": "Потрібна назва, під якою її збережено в DSP.",
        "cfgTooOld": "Метод на цьому комп'ютері ще не вміє записувати конфігурації. Онови метод.",
        "cfgNoAnswer": "Метод не відповів, тож нічого не записано.",
        "cfgSaved": "Записано: {said}",
        "dspConfigTip": "Збережено в DSP як {code} (слот {slot}{preset}){purpose}.",
        "dspConfigPreset": ", пресет пристрою {n}",
        "cmpPrevTip": "За замовчуванням порівнюється з конфігурацією, яку ця продовжує, а не з номером нижче.",
        "gitSubBroken": "git не працює",
        "gitSubNoRepo": "історія не зберігається",
        "gitSubNoRemote": "без резервної копії",
        "gitSubBehind": "не відправлено: {n}",
        "gitSubOk": "копія є",
        "gitTipBroken": "git на цьому комп'ютері не запускається, тож історія налаштування не зберігається і нічого не копіюється. На Mac: встанови Command Line Tools (xcode-select --install).",
        "gitTipNoRepo": "Тека проєкту не є git-репозиторієм: історія налаштування не зберігається і резервної копії нема. Новий проєкт метод робить репозиторієм сам; для цього — кнопка «Зробити репозиторій» нижче запускає його команду.",
        "gitTipNoRemote": "Історія зберігається тут, але резервної копії нема: у репозиторію нема віддаленого. Безкоштовний приватний на GitHub: gh repo create {name} --private --source . --push",
        "gitTipBehind": "{n} комітів ще нема на {remote}: git push.",
        "gitTipOk": "Історія зберігається, копія на {remote}.",
        "gitRow": "Git",
        "gitChanges": "Незбережені зміни",
        "gitClean": "нема",
        "gitBackup": "Резервна копія",
        "gitNone": "нема",
        "gitUnpushed": "Ще не відправлено",
        "gitInitBtn": "Зробити репозиторій",
        "gitInitTip": "Запускає команду самого методу: git init, .gitignore, перший коміт. Нічого не залишає цей комп'ютер.",
        "gitBackupBtn": "Копія на GitHub",
        "gitBackupTip": "Показує команду методу для безкоштовного приватного репозиторію на GitHub і запускає її лише після твого «так».",
        "gitBackupAsk": "Створити приватний репозиторій на GitHub і відправити туди історію проєкту?\n\n{command}\n\nЗаміри (.mdat) і ключі лишаються на цьому комп'ютері.",
        "gitDone": "Зроблено: {said}",
        "gitFailed": "Не вийшло: {said}",
        "gitTooOld": "Метод на цьому комп'ютері ще цього не вміє. Онови метод.",
        "gitNoGh": "gh від GitHub не встановлено, тож копію запропонувати не можна. Історія все одно зберігається тут.",
        "gitGhSignedOut": "gh не увійшов: виконай у терміналі gh auth login --web і спробуй ще раз.",
        "extTerminal": "⌁ у терміналі: {who} · {model}",
        "extNoModel": "модель не названа",
        "extTip": "Сесія поза цим вікном під'єднана до TCC ({client} {version}); модель — як сесія назвала себе сама. Вибір поруч — те, з чим TCC відкриє нову сесію.",
        "hoOffer": "У фазі {phase} не лишилось відкритих кроків. Наступну фазу краще почати в чистій сесії.",
        "hoAction": "Перевірити й почати начисто",
        "hoTitle": "Наступна фаза — у чистій сесії",
        "hoMissing": "Ще ні: наступній сесії бракуватиме того, що є лише в цій розмові.\n\n{missing}\n\nКожен рядок каже, що це виправляє.",
        "hoReady": "Усе, що потрібно наступній сесії, записано.\n\nПочни її з першого повідомлення «{message}» — воно буде в буфері обміну.\nЛишити відкритим: {resume}",
        "hoStartInApp": "Почати чисту сесію",
        "hoStartTerminal": "Відкрити термінал",
        "hoCopied": "«{message}» у буфері обміну: надішли його першим повідомленням новій сесії, а стару закрий.",
        "hoTooOld": "Метод на цьому комп'ютері ще не вміє перевіряти перехід. Онови метод.",
        "tfOffer": "Назва в REW записана не так, як у раунді: {first} (усього {n}). Це виправляється перейменуванням, а не перезніманням.",
        "tfAction": "Виправити в REW…",
        "dspNowTip": "{dsp} — конфігурація, що зараз у процесорі: {version} пресету {preset}. Введено в DSP: {applied}; лише запропоновано, ще не введено: {proposed}; після того виміряно: {measured}. Крапка жовта, доки щось лише запропоновано, і зелена, коли все введено (attest).",
        ...
        "menuReviewerKey": "Ключ рецензента…",
        "menuReviewerKeyTip": "Де лежить API-ключ рецензента, і безпечне місце, щоб його ввести. Рецензентові, що увійшов через свій CLI, ключ не потрібен.",
        "rkTitle": "Ключ рецензента",
        "rkBlurb": "Ключ для API-шляху рецензента. Метод тримає його в {store}: ніколи в профілі термінала і ніколи в теці проєкту. Рецензентові, що увійшов через свій CLI (agy, claude, codex), ключ не потрібен узагалі. Сам ключ тут ніколи не показується.",
        "rkStore_keychain": "Keychain цього Мака",
        "rkStore_dpapi": "файлі, зашифрованому твоїм входом у Windows",
        "rkStore_none": "файлі critic-env, який читаєш лише ти (захищеного сховища, яким метод уміє користуватись, на цьому комп'ютері нема)",
        "rkUsed_keystore": "у захищеному сховищі",
        "rkUsed_file": "у файлі critic-env",
        "rkUsed_env": "зі змінної середовища: його бачить кожна програма",
        "rkUsed_none": "ключа нема",
        "rkPlaceholder": "Встав ключ. Його не видно",
        "rkSave": "Зберегти",
        "rkClose": "Закрити",
        "rkSaved": "Збережено: {where}",
        "rkRefused": "Не збережено: {why}",
        "rkNoAnswer": "Метод не відповів, тож нічого не збережено.",
        "rkShell": "{var} досі експортується у {file}, рядок {line}. Його бачить кожна програма, запущена з термінала. Метод переносить його в захищене сховище і спершу питає тебе:",
        "rkMove": "Перенести (відкриє термінал)",
        "rkMoveOpened": "Відкрито в терміналі. Дай відповідь там, потім відкрий це вікно знову.",
        "rkOld": "Метод на цьому комп'ютері ще не вміє зберігати ключі, тож читає їх із {path} або зі змінних середовища. Онови метод; доти нічого не змінюється.",
        ...
        "riSendFirst": "Імпорт з проєкту Resonalyze — {file}. Звірено з профілем DSP цього проєкту: {ok} значень заходять, жодного відмовленого, {unknown} не перевірених. Реєстру в проєкті ще нема, тож запиши це ПЕРШОЮ конфігурацією (версія реєстру v_001) пресету {preset}, через гейт. Нижче рядки, по каналах:",
        ...
        "capImportProposed": "Нове ім'я підібрано автоматично (як у раунді, або схоже на одруківку). Постав галочку, якщо ти мав на увазі саме цю назву.",
        ...
        "chanToggleWaitTip": "TCC попросив модель це записати; реєстр пише скіл, не TCC. Рядок зміниться, коли модель відповість. Повторний клік лише оновлює очікування, другого запиту не надсилає.",
        "chanToggleAlreadyAsked": "{channel} — уже запитано, чекаю на модель. Вдруге не надсилав.",
        "chanToggleTip": "Попросити модель увімкнути або вимкнути канал. TCC не пише реєстр — запит іде в сесію, і зміну записує вона.",
        "chanToggleSent": "Попросив перемкнути <b>{channel}</b> → {state}. Модель запише це в реєстр; дерево оновиться, щойно запис буде.",
        "noSessionForSignal": "Сесія не запущена — запусти, і запит до неї дійде.",
        "chanToggleConfirmTitle": "Перемкнути канал?",
        "chanToggleConfirmOff": "Вимкнути <b>{channel}</b>?\n\nЙого EQ, кросовер і затримка живуть у реєстрі й можуть не пережити вимкнення. TCC це не відкотить — зміну записує модель.",
...
ще раз; якщо повторюється — причина тут і в лозі:",
        ...
        "menuGuide": "Посібник користувача",
        "menuGuideTip": "Відкриває посібник для цієї версії на GitHub: вікно панель за панеллю, зі знімками екрана.",
...
Щоб це читати, обери вище заміри свіпом (sw).",
        ...
        "editStartForgot": "◆ Правка параметрів проєкту — позначено «скіл не зберіг». Сесії надіслано сигнал: опиши, що має бути в реєстрі, — вона перевірить і запише.",
        "editStartManual": "◆ Правка параметрів проєкту — «я змінив щось руками». Сесії надіслано сигнал: скажи, що й де змінено, — вона запише це в реєстр.",
        "editDoneNoBus": "✓ Правку закрито — нічого не надіслано: сесія не слухає.",
    ...
    "pl": {
        ...
        "npOnboardingHint": "Skorzystaj ze skilla autosound-tuning. Intake tego projektu wypełniono w formularzu, więc najpierw przeczytaj pliki projektu i kontynuuj od tego, czego metoda jeszcze nie ma (contract.py check). Połącz się z serwerem MCP 'tcc' tego projektu (zob. .mcp.json) i wywołuj recenzenta przez jego narzędzie call_critic, a nie uruchamiając autosound_ai.py. Pracuj {language}.",
        ...
        "layoutControl": "Tryb kontroli",
        "layoutActive": "Aktywny TCC",
        "layoutControlTip": "Sesja działa w terminalu: TCC przechodzi na prawą połowę ekranu i pokazuje, co widzi z sesji, tabele i twoje panele. Obie granice da się przeciągać i są zapamiętywane.",
        "layoutActiveTip": "Z powrotem do układu sesji w TCC: dialog pośrodku, panele po bokach.",
        "layoutBusyTip": "W tym oknie trwa sesja — najpierw ją zatrzymaj: tryb kontroli ukrywa jej dialog.",
        "layoutNoInApp": "Tryb kontroli jest dla sesji w terminalu — przełącz na „Aktywny TCC”, żeby uruchomić ją tutaj.",
        "ctlMonitor": "Monitoring",
        "ctlTableV": "Tabela-V",
        "ctlTableO": "Tabela-O",
        "ctlMonitorNote": "Piszesz w terminalu. Te przyciski idą do sesji jako sygnały.",
        "ctlMonitorEmpty": "Nie ma jeszcze dziennika procesu — pasek wypełni się, gdy sesja w terminalu zacznie pisać.",
        "ctlNoVirtual": "Ten DSP nie ma kanałów wirtualnych.",
        "ctlNoOutputs": "Nie ma jeszcze wyjść — rejestr nie ma konfiguracji.",
        "measNextRound": "nowa runda",
        "measRoundSeriesTip": "Seria _{v}: numer, który te pomiary mają w nazwach w REW.",
        "cmpWith": "porównaj z",
        "cmpWas": "było: {value}",
        "cmpNew": "nie ma tego w porównywanej wersji",
        "dotNone": "nic tu nie ustawiono",
        "dotSet": "są ustawienia",
        "dotChg": "zmiany względem {version}",
        "ctlTableI": "Tabela-I",
        "cmpOtherPreset": "{preset} — inny preset",
        "cmpOtherTag": "inna konfiguracja",
        "cmpEqChanged": "pasma się zmieniły",
        "cmpRowBtn": "⇅ Porównaj",
        "cmpRowTip": "Pokaż EQ {version} pod bieżącym, zmiany kolorem",
        "cmpRowOff": "Najpierw wybierz wersję w «porównaj z»",
        "bandNew": "nowe",
        "bandChg": "zmienione",
        "bandGone": "usunięte",
        "bandNewTip": "nie ma go w {version}",
        "bandGoneTip": "nie ma go w bieżącej wersji",
        "copyEqPairTip": "Na ekranie są dwa kanały: kopiowanie każdego jest przy jego nazwie.",
        "cfgButton": "Zapisano w DSP…",
        "cfgButtonTip": "Zapisz, że ta konfiguracja trafiła do presetu urządzenia pod nazwą, którą jej tam nadałeś (SQ-2, FULL-v1). Tę nazwę pokazują porównanie i nagłówek.",
        "cfgTitle": "Zapisano w DSP",
        "cfgBlurb": "{version} trafiła do presetu urządzenia. Pod jaką nazwą, do którego slotu i numeru presetu, i w jakim celu? Którą wcześniejszą konfigurację kontynuuje, metoda znajdzie sama.",
        "cfgName": "Nazwa w DSP",
        "cfgSlot": "Slot",
        "cfgDspPreset": "Numer presetu w urządzeniu",
        "cfgPurpose": "W jakim celu",
        "cfgPurposeHint": "jakość dźwięku, całe auto, miejsce kierowcy…",
        "cfgSave": "Zapisz",
        "cfgNoName": "Potrzebna jest nazwa, pod którą zapisano ją w DSP.",
        "cfgTooOld": "Metoda na tym komputerze nie umie jeszcze zapisywać konfiguracji. Zaktualizuj metodę.",
        "cfgNoAnswer": "Metoda nie odpowiedziała, więc nic nie zapisano.",
        "cfgSaved": "Zapisano: {said}",
        "dspConfigTip": "Zapisano w DSP jako {code} (slot {slot}{preset}){purpose}.",
        "dspConfigPreset": ", preset urządzenia {n}",
        "cmpPrevTip": "Domyślnie porównywana z konfiguracją, którą ta kontynuuje, a nie z numerem niżej.",
        "gitSubBroken": "git nie działa",
        "gitSubNoRepo": "historia nie jest zapisywana",
        "gitSubNoRemote": "bez kopii zapasowej",
        "gitSubBehind": "niewysłane: {n}",
        "gitSubOk": "kopia jest",
        "gitTipBroken": "git nie uruchamia się na tym komputerze, więc historia strojenia nie jest zapisywana i nic nie jest kopiowane. Na Macu: zainstaluj Command Line Tools (xcode-select --install).",
        "gitTipNoRepo": "Folder projektu nie jest repozytorium git: historia strojenia nie jest zapisywana i nie ma kopii zapasowej. Nowy projekt metoda robi repozytorium sama; dla tego — przycisk „Zrób repozytorium” poniżej uruchamia jej polecenie.",
        "gitTipNoRemote": "Historia jest zapisywana tutaj, ale nie ma kopii zapasowej: repozytorium nie ma zdalnego. Darmowe prywatne na GitHubie: gh repo create {name} --private --source . --push",
        "gitTipBehind": "{n} commitów nie ma jeszcze na {remote}: git push.",
        "gitTipOk": "Historia jest zapisywana, kopia na {remote}.",
        "gitRow": "Git",
        "gitChanges": "Niezapisane zmiany",
        "gitClean": "brak",
        "gitBackup": "Kopia zapasowa",
        "gitNone": "brak",
        "gitUnpushed": "Jeszcze niewysłane",
        "gitInitBtn": "Zrób repozytorium",
        "gitInitTip": "Uruchamia polecenie samej metody: git init, .gitignore, pierwszy commit. Nic nie opuszcza tego komputera.",
        "gitBackupBtn": "Kopia na GitHubie",
        "gitBackupTip": "Pokazuje polecenie metody dla darmowego prywatnego repozytorium na GitHubie i uruchamia je dopiero po Twoim „tak”.",
        "gitBackupAsk": "Utworzyć prywatne repozytorium na GitHubie i wysłać tam historię projektu?\n\n{command}\n\nPomiary (.mdat) i klucze zostają na tym komputerze.",
        "gitDone": "Gotowe: {said}",
        "gitFailed": "Nie udało się: {said}",
        "gitTooOld": "Metoda na tym komputerze jeszcze tego nie umie. Zaktualizuj metodę.",
        "gitNoGh": "gh od GitHuba nie jest zainstalowany, więc nie można zaproponować kopii. Historia i tak jest zapisywana tutaj.",
        "gitGhSignedOut": "gh nie jest zalogowany: uruchom w terminalu gh auth login --web i spróbuj ponownie.",
        "extTerminal": "⌁ w terminalu: {who} · {model}",
        "extNoModel": "model nienazwany",
        "extTip": "Sesja poza tym oknem jest połączona z TCC ({client} {version}); model — tak, jak sesja sama się nazwała. Wybór obok to to, z czym TCC otworzy nową sesję.",
        "hoOffer": "W fazie {phase} nie został żaden otwarty krok. Następną fazę najlepiej zacząć w czystej sesji.",
        "hoAction": "Sprawdź i zacznij od nowa",
        "hoTitle": "Następna faza — w czystej sesji",
        "hoMissing": "Jeszcze nie: następnej sesji brakowałoby czegoś, co jest tylko w tej rozmowie.\n\n{missing}\n\nKażdy wiersz mówi, co to naprawia.",
        "hoReady": "Wszystko, czego potrzebuje następna sesja, jest zapisane.\n\nZacznij ją od pierwszej wiadomości „{message}” — trafi do schowka.\nZostaw otwarte: {resume}",
        "hoStartInApp": "Zacznij czystą sesję",
        "hoStartTerminal": "Otwórz terminal",
        "hoCopied": "„{message}” jest w schowku: wyślij je jako pierwszą wiadomość nowej sesji, a starą zamknij.",
        "hoTooOld": "Metoda na tym komputerze nie umie jeszcze sprawdzić przejścia. Zaktualizuj metodę.",
        "tfOffer": "Nazwa w REW jest zapisana inaczej niż w rundzie: {first} (razem {n}). Naprawia to zmiana nazwy, nie ponowny pomiar.",
        "tfAction": "Popraw w REW…",
        "dspNowTip": "{dsp} — konfiguracja, która jest teraz w procesorze: {version} presetu {preset}. Wprowadzone do DSP: {applied}; tylko zaproponowane, jeszcze niewprowadzone: {proposed}; zmierzone od tego czasu: {measured}. Kropka jest żółta, dopóki coś jest tylko zaproponowane, i zielona, gdy wszystko wprowadzono (attest).",
        "menuCopyCarTip": 'Zacznij projekt od już istniejącego: auto, sprzęt i montaż — marka, głośniki po kanałach, '
                          ...
                          'od nowa.',
        "menuReviewerKey": "Klucz recenzenta…",
        "menuReviewerKeyTip": "Gdzie jest przechowywany klucz API recenzenta i bezpieczne miejsce, by go wpisać. Recenzent zalogowany przez własne CLI nie potrzebuje klucza.",
        "rkTitle": "Klucz recenzenta",
        "rkBlurb": "Klucz dla ścieżki API recenzenta. Metoda trzyma go w {store}: nigdy w profilu powłoki i nigdy w folderze projektu. Recenzent zalogowany przez własne CLI (agy, claude, codex) w ogóle nie potrzebuje klucza. Sam klucz nigdy nie jest tu pokazywany.",
        "rkStore_keychain": "pęku kluczy (Keychain) tego Maca",
        "rkStore_dpapi": "pliku zaszyfrowanym Twoim logowaniem do Windows",
        "rkStore_none": "pliku critic-env, który czytasz tylko Ty (na tym komputerze nie ma bezpiecznego magazynu, którego metoda umie użyć)",
        "rkUsed_keystore": "w bezpiecznym magazynie",
        "rkUsed_file": "w pliku critic-env",
        "rkUsed_env": "ze zmiennej środowiskowej: widzi go każdy program",
        "rkUsed_none": "brak klucza",
        "rkPlaceholder": "Wklej klucz. Pozostaje ukryty",
        "rkSave": "Zapisz",
        "rkClose": "Zamknij",
        "rkSaved": "Zapisano: {where}",
        "rkRefused": "Nie zapisano: {why}",
        "rkNoAnswer": "Metoda nie odpowiedziała, więc nic nie zapisano.",
        "rkShell": "{var} jest nadal eksportowany w {file}, wiersz {line}. Widzi go każdy program uruchomiony z terminala. Metoda przenosi go do bezpiecznego magazynu i najpierw Cię pyta:",
        "rkMove": "Przenieś (otworzy terminal)",
        "rkMoveOpened": "Otwarto w terminalu. Odpowiedz tam na pytanie, potem otwórz to okno ponownie.",
        "rkOld": "Metoda na tym komputerze nie umie jeszcze przechowywać kluczy, więc czyta je z {path} lub ze środowiska. Zaktualizuj metodę; do tego czasu nic się nie zmienia.",
        ...
        "capImportProposed": "Nowa nazwa została dopasowana automatycznie (jak w rundzie albo prawdopodobna literówka). Zaznacz wiersz, jeśli chodziło o tę nazwę.",
        ...
        "menuGuide": "Podręcznik użytkownika",
        "menuGuideTip": "Otwiera podręcznik dla tej wersji na GitHub: okno panel po panelu, ze zrzutami ekranu.",
        ...
        "editDoneNoBus": "✓ Edycję zamknięto — nic nie wysłano: żadna sesja nie słucha.",
    ...
    "de": {
        ...
        "npOnboardingHint": "Nutze den Skill autosound-tuning. Das Intake dieses Projekts wurde im Formular ausgefüllt; lies zuerst die Projektdateien und mach dort weiter, wo die Methode noch etwas vermisst (contract.py check). Verbinde dich mit dem MCP-Server 'tcc' dieses Projekts (siehe .mcp.json) und ruf den Reviewer über dessen Werkzeug call_critic auf, nicht durch Starten von autosound_ai.py. Arbeite {language}.",
        ...
        "layoutControl": "Kontrollmodus",
        "layoutActive": "TCC aktiv",
        "layoutControlTip": "Die Sitzung läuft im Terminal: TCC geht auf die rechte Bildschirmhälfte und zeigt, was es von der Sitzung sieht, die Tabellen und deine Panels. Beide Grenzen lassen sich ziehen und werden gemerkt.",
        "layoutActiveTip": "Zurück zum Layout der Sitzung in TCC: der Dialog in der Mitte, die Panels an den Seiten.",
        "layoutBusyTip": "In diesem Fenster läuft eine Sitzung — beende sie zuerst: der Kontrollmodus verbirgt ihren Dialog.",
        "layoutNoInApp": "Der Kontrollmodus ist für eine Sitzung im Terminal — wechsle zu „TCC aktiv“, um sie hier zu starten.",
        "ctlMonitor": "Monitoring",
        "ctlTableV": "Tabelle-V",
        "ctlTableO": "Tabelle-O",
        "ctlMonitorNote": "Du schreibst im Terminal. Diese Knöpfe gehen als Signale an die Sitzung.",
        "ctlMonitorEmpty": "Noch kein Prozessjournal — der Verlauf füllt sich, sobald die Sitzung im Terminal zu schreiben beginnt.",
        "ctlNoVirtual": "Dieses DSP hat keine virtuellen Kanäle.",
        "ctlNoOutputs": "Noch keine Ausgänge — das Register hat keine Konfiguration.",
        "measNextRound": "nächste Runde",
        "measRoundSeriesTip": "Serie _{v}: die Nummer, die diese Messungen in ihren REW-Titeln tragen.",
        "cmpWith": "vergleichen mit",
        "cmpWas": "war: {value}",
        "cmpNew": "in der verglichenen Version nicht vorhanden",
        "dotNone": "hier ist nichts eingestellt",
        "dotSet": "Einstellungen vorhanden",
        "dotChg": "geändert gegenüber {version}",
        "ctlTableI": "Tabelle-I",
        "cmpOtherPreset": "{preset} — anderes Preset",
        "cmpOtherTag": "andere Konfiguration",
        "cmpEqChanged": "die Bänder haben sich geändert",
        "cmpRowBtn": "⇅ Vergleichen",
        "cmpRowTip": "Das EQ von {version} unter diesem zeigen, Änderungen farbig",
        "cmpRowOff": "Zuerst eine Version unter «vergleichen mit» wählen",
        "bandNew": "neu",
        "bandChg": "geändert",
        "bandGone": "entfernt",
        "bandNewTip": "nicht in {version}",
        "bandGoneTip": "nicht in der aktuellen Version",
        "copyEqPairTip": "Zwei Kanäle sind zu sehen: das Kopieren jedes Kanals steht neben seinem Namen.",
        "cfgButton": "Im DSP gespeichert…",
        "cfgButtonTip": "Festhalten, dass diese Konfiguration unter dem Namen, den du ihr dort gegeben hast (SQ-2, FULL-v1), in ein Preset des Geräts gegangen ist. Diesen Namen zeigen der Vergleich und die Kopfzeile.",
        "cfgTitle": "Im DSP gespeichert",
        "cfgBlurb": "{version} ist in ein Preset des Geräts gegangen. Unter welchem Namen, in welchen Slot und welche Presetnummer, und wofür? Welche frühere Konfiguration sie fortsetzt, findet die Methode selbst.",
        "cfgName": "Name im DSP",
        "cfgSlot": "Slot",
        "cfgDspPreset": "Presetnummer im Gerät",
        "cfgPurpose": "Wofür",
        "cfgPurposeHint": "Klangqualität, das ganze Auto, Fahrerplatz…",
        "cfgSave": "Festhalten",
        "cfgNoName": "Es braucht den Namen, unter dem sie im DSP gespeichert wurde.",
        "cfgTooOld": "Die Methode auf diesem Computer kann noch keine Konfigurationen festhalten. Aktualisiere die Methode.",
        "cfgNoAnswer": "Die Methode hat nicht geantwortet, daher wurde nichts festgehalten.",
        "cfgSaved": "Festgehalten: {said}",
        "dspConfigTip": "Im DSP gespeichert als {code} (Slot {slot}{preset}){purpose}.",
        "dspConfigPreset": ", Geräte-Preset {n}",
        "cmpPrevTip": "Standardmäßig mit der Konfiguration verglichen, die diese fortsetzt, nicht mit der Nummer darunter.",
        "gitSubBroken": "git läuft nicht",
        "gitSubNoRepo": "keine Historie",
        "gitSubNoRemote": "keine Sicherung",
        "gitSubBehind": "nicht gesichert: {n}",
        "gitSubOk": "gesichert",
        "gitTipBroken": "git läuft auf diesem Computer nicht, daher wird die Historie der Abstimmung nicht gespeichert und nichts gesichert. Auf dem Mac: die Command Line Tools installieren (xcode-select --install).",
        "gitTipNoRepo": "Der Projektordner ist kein git-Repository: Die Historie der Abstimmung wird nicht gespeichert, und es gibt keine Sicherung. Ein neues Projekt legt die Methode selbst als Repository an; für dieses führt „Repository anlegen“ unten ihren Befehl aus.",
        "gitTipNoRemote": "Die Historie wird hier gespeichert, aber es gibt keine Sicherung: Das Repository hat kein Remote. Ein kostenloses privates auf GitHub: gh repo create {name} --private --source . --push",
        "gitTipBehind": "{n} Commits sind noch nicht auf {remote}: git push.",
        "gitTipOk": "Die Historie wird gespeichert und auf {remote} gesichert.",
        "gitRow": "Git",
        "gitChanges": "Ungespeicherte Änderungen",
        "gitClean": "keine",
        "gitBackup": "Sicherung",
        "gitNone": "keine",
        "gitUnpushed": "Noch nicht gesichert",
        "gitInitBtn": "Repository anlegen",
        "gitInitTip": "Führt den Befehl der Methode selbst aus: git init, .gitignore, ein erster Commit. Nichts verlässt diesen Computer.",
        "gitBackupBtn": "Auf GitHub sichern",
        "gitBackupTip": "Zeigt den Befehl der Methode für ein kostenloses privates Repository auf GitHub und führt ihn erst nach deinem Ja aus.",
        "gitBackupAsk": "Ein privates Repository auf GitHub anlegen und die Historie des Projekts dorthin senden?\n\n{command}\n\nMessungen (.mdat) und Schlüssel bleiben auf diesem Computer.",
        "gitDone": "Erledigt: {said}",
        "gitFailed": "Hat nicht geklappt: {said}",
        "gitTooOld": "Die Methode auf diesem Computer kann das noch nicht. Aktualisiere die Methode.",
        "gitNoGh": "GitHubs gh ist nicht installiert, daher kann keine Sicherung angeboten werden. Die Historie wird trotzdem hier gespeichert.",
        "gitGhSignedOut": "gh ist nicht angemeldet: im Terminal gh auth login --web ausführen und es erneut versuchen.",
        "extTerminal": "⌁ im Terminal: {who} · {model}",
        "extNoModel": "Modell nicht genannt",
        "extTip": "Eine Sitzung außerhalb dieses Fensters ist mit TCC verbunden ({client} {version}); das Modell so, wie die Sitzung sich selbst genannt hat. Die Auswahl daneben ist das, womit TCC eine neue Sitzung öffnet.",
        "hoOffer": "In Phase {phase} ist kein Schritt mehr offen. Die nächste Phase beginnt man am besten in einer frischen Sitzung.",
        "hoAction": "Prüfen und frisch beginnen",
        "hoTitle": "Die nächste Phase, in einer frischen Sitzung",
        "hoMissing": "Noch nicht: der nächsten Sitzung würde etwas fehlen, das nur in diesem Gespräch steht.\n\n{missing}\n\nJede Zeile nennt, was es behebt.",
        "hoReady": "Alles, was die nächste Sitzung braucht, ist gespeichert.\n\nBeginne sie mit der ersten Nachricht „{message}“ — sie liegt in der Zwischenablage.\nOffen lassen: {resume}",
        "hoStartInApp": "Frische Sitzung starten",
        "hoStartTerminal": "Terminal öffnen",
        "hoCopied": "„{message}“ ist in der Zwischenablage: als erste Nachricht der neuen Sitzung senden und die alte schließen.",
        "hoTooOld": "Die Methode auf diesem Computer kann den Übergang noch nicht prüfen. Aktualisiere die Methode.",
        "tfOffer": "Ein Titel in REW ist anders geschrieben als in der Runde: {first} (insgesamt {n}). Das behebt eine Umbenennung, keine neue Messung.",
        "tfAction": "In REW korrigieren…",
        "dspNowTip": "{dsp} — die Konfiguration, die jetzt im Prozessor ist: {version} des Presets {preset}. Im DSP eingetragen: {applied}; nur vorgeschlagen, noch nicht eingetragen: {proposed}; seitdem gemessen: {measured}. Der Punkt ist gelb, solange etwas nur vorgeschlagen ist, und grün, sobald alles eingetragen ist (attest).",
        ...
        "menuReviewerKey": "Schlüssel des Prüfers…",
        "menuReviewerKeyTip": "Wo der API-Schlüssel des Prüfers liegt, und ein sicherer Ort, um einen einzugeben. Ein Prüfer, der über sein eigenes CLI angemeldet ist, braucht keinen Schlüssel.",
        "rkTitle": "Schlüssel des Prüfers",
        "rkBlurb": "Der Schlüssel für den API-Weg des Prüfers. Die Methode bewahrt ihn in {store} auf: nie in einem Shell-Profil, nie im Projektordner. Ein Prüfer, der über sein eigenes CLI (agy, claude, codex) angemeldet ist, braucht überhaupt keinen Schlüssel. Der Schlüssel selbst wird hier nie angezeigt.",
        "rkStore_keychain": "der Schlüsselbund (Keychain) dieses Macs",
        "rkStore_dpapi": "einer Datei, die mit deiner Windows-Anmeldung verschlüsselt ist",
        "rkStore_none": "der Datei critic-env, die nur du lesen kannst (dieser Computer hat keinen sicheren Speicher, den die Methode nutzen kann)",
        "rkUsed_keystore": "im sicheren Speicher",
        "rkUsed_file": "in der Datei critic-env",
        "rkUsed_env": "aus der Umgebung: jedes Programm sieht ihn",
        "rkUsed_none": "kein Schlüssel",
        "rkPlaceholder": "Schlüssel einfügen. Er bleibt verborgen",
        "rkSave": "Speichern",
        "rkClose": "Schließen",
        "rkSaved": "Gespeichert: {where}",
        "rkRefused": "Nicht gespeichert: {why}",
        "rkNoAnswer": "Die Methode hat nicht geantwortet, daher wurde nichts gespeichert.",
        "rkShell": "{var} wird noch in {file}, Zeile {line}, exportiert. Jedes aus einem Terminal gestartete Programm sieht ihn. Die Methode verschiebt ihn in den sicheren Speicher und fragt dich vorher:",
        "rkMove": "Verschieben (öffnet ein Terminal)",
        "rkMoveOpened": "Im Terminal geöffnet. Beantworte die Frage dort und öffne dieses Fenster dann erneut.",
        "rkOld": "Die Methode auf diesem Computer kann noch keine Schlüssel speichern und liest sie daher aus {path} oder der Umgebung. Aktualisiere die Methode; bis dahin ändert sich nichts.",
        ...
        "capImportProposed": "Der neue Name wurde automatisch zugeordnet (wie in der Runde oder ein wahrscheinlicher Tippfehler). Hake die Zeile an, wenn du genau diesen Namen meinst.",
        ...
        "menuGuideTip": "Öffnet das Handbuch für diese Version auf GitHub: das Fenster, Panel für Panel, mit Screenshots.",
        ...
        "editDoneNoBus": "✓ Bearbeitung geschlossen — nichts gesendet: keine Sitzung hört zu.",
